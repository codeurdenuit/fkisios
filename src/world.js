...
export default class World extends Object3D {
  ...
  constructor(meshesSolid, meshesCollider, physic) {
    ...
    this.initLights()
    const ambient = new Audio('sound/ambient.mp3')
    ambient.loop = true
    ambient.volume = 0.5
    const music = new Audio('sound/AddingTheSun.mp3')
    music.loop = true
    music.volume = 0.4
    ambient.play()
    music.play()
  ...
  initVisual(meshes) {
    for(const mesh of meshes) {
      if (mesh.name.includes('tree'))
        replaceMaterial(mesh, materialTree)
      this.add(mesh)
    }
  }
  ...
  update(dt, Player) {
    ...
    if(player&&player.active){
      this.dirLight.position.set(player.position.x-10,20,player.position.z+10)    
      this.dirLight.target.position.set(player.position.x, 0, player.position.z-5)
    }
